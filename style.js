...
let records = JSON.parse(localStorage.getItem("localdata1")) || [];
let isEdit = -1;
let renderTable = () => {
...
function validateForm() {
  ...
  if (isEdit !== -1) {
    ...
    records = updated;
    localStorage.setItem('localdata1', JSON.stringify(updated))
    renderTable()
    isEdit - 1;
  }
  else {
    records.push(person);
    localStorage.setItem("localdata1", JSON.stringify(records));
    renderTable();
  }
  console.log(records);
...
function handledelete(idx) {
  let deletedData = records?.filter((item, index) => { return (index !== idx) });
  records = deletedData;
  renderTable();
  localStorage.setItem("localdata1", JSON.stringify(deletedData))
